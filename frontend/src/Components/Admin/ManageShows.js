...
import { Plus, Trash2, ArrowLeft, Calendar, Clock, MapPin, Film } from 'lucide-react';
...
export const ManageShows = ({ onBack }) => {
    ...
    const fetchShows = async () => {
        try {
            ...
            const res = await fetch('http://localhost:5000/api/shows?date=' + new Date().toISOString().split('T')[0]); // Initial fetch for today, can improve
    ...
    const handleAdd = async (e) => {
        ...
        if (data.success) {
            ...
            fetchShows();
            setNewShow({ movieId: '', theaterId: '', date: '', time: '', price: '' });
        } else {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-8">
            <button onClick={onBack} className="flex items-center gap-2 mb-6 text-blue-600 font-medium hover:underline">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                    <h2 className="text-xl font-bold mb-6 flex items-center gap-2 text-gray-800">
                        <Plus size={20} className="text-blue-500" /> Schedule New Show
                    </h2>
                    ...
                    <form onSubmit={handleAdd} className="space-y-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setNewShow({ ...newShow, theaterId: e.target.value })}
                                required
                            >
                        ...
                        <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold text-lg hover:bg-blue-700 transition shadow-md hover:shadow-lg transform active:scale-95">
                            Schedule Show
                        </button>
                    </form>
                </div>
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-lg border border-gray-100 flex flex-col h-[600px]">
                    <h2 className="text-xl font-bold mb-6 text-gray-800">Scheduled Shows</h2>

                    <div className="flex-1 overflow-y-auto pr-2">
                        {shows.length === 0 ? (
                            <p className="text-center text-gray-400 mt-10">No shows scheduled yet.</p>
                        ) : (
                            <div className="space-y-4">
                                {shows.map(show => (
                                    <div key={show._id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-blue-200 transition group">
                                        <div className="flex gap-4">
                                            <div className="w-16 h-20 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0">
                                                ...
                                                {show.movie && show.movie.poster && <img src={show.movie.poster} alt="" className="w-full h-full object-cover" />}
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-gray-900">{show.movie ? show.movie.title : 'Unknown Movie'}</h3>
                                                <div className="flex items-center gap-4 text-sm text-gray-500 mt-1">
                                                    <span className="flex items-center gap-1"><MapPin size={14} /> {show.theater ? show.theater.name : 'Unknown Theater'}</span>
                                                    <span className="flex items-center gap-1"><Calendar size={14} /> {new Date(show.date).toLocaleDateString()}</span>
