                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
                    ...
                    <div className="md:col-span-5">
                        ...
                        <div className="inline-flex items-center gap-4 bg-white/5 backdrop-blur-md p-2 pr-6 rounded-2xl border border-white/10 hover:border-purple-500/30 transition-all group cursor-pointer">
                            ...
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Support Email</p>
                                <p className="text-sm font-bold text-slate-200">bookings@movietix.com</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 md:pt-4">
                        ...
                        <ul className="space-y-4">
                            <FooterLink to="/upcoming-movies" label="Upcoming Movies" />
                    ...
                    <div className="md:col-span-2 md:pt-4">
                        ...
                        <ul className="space-y-4">
                            ...
                            <FooterLink to="/booking-guide" label="Booking Guide" />
                    ...
                    <div className="md:col-span-3 md:pt-4">
                        ...
                        <p className="mt-6 text-xs font-bold text-slate-500 leading-relaxed">
                            Follow us for exclusive early access to premieres and special fan events.
                        </p>
                    </div>
                </div>

                {/* Stats Belt */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-12 border-y border-white/5 mb-12 bg-white/[0.02] rounded-[2rem] px-8 backdrop-blur-sm">
                    <StatItem value="5M+" label="Happy Customers" color="text-amber-400" />
                    <StatItem value="7+" label="Premium Theaters" color="text-emerald-400" />
                    <StatItem value="50+" label="Cities Covered" color="text-blue-400" />
                    <StatItem value="10M+" label="Tickets Booked" color="text-purple-400" />
                </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="text-center md:text-left order-2 md:order-1">
                        <p className="text-slate-500 text-sm font-bold">
                            © 2026 <span className="text-white font-black tracking-tight">MOVIETIX</span>. All rights reserved. 
                        </p>
                    </div>
                    
                    <div className="flex items-center gap-8 order-1 md:order-2">
                        <Link to="/" className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors">Privacy Policy</Link>
                        <Link to="/" className="text-xs font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors">Terms of Service</Link>
                    </div>
                </div>
            </div>
        </footer>
...
const FooterLink = ({ to, label }) => (
    <li>
        ...
        >
            <ChevronRight size={14} className="text-purple-600 opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0 transition-all" />
            <span>{label}</span>
        </Link>
    </li>
);
...
const SocialIcon = ({ icon }) => (
    <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:bg-purple-600 hover:text-white hover:border-purple-600 hover:-translate-y-1 transition-all duration-300 cursor-pointer shadow-lg">
        {icon}
    </div>
);
